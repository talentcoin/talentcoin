...
#if defined(NDEBUG)
# error "TALENT cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xca4b1b02ec0c4d2531398f059ba3262b72946ca061585e4a0f4ec0807d6785a6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TALENT: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TALENT Signed Message:\n";
...
{
    // TALENT: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // TALENT: 1 days
static const int64 nTargetSpacing = 3600; // TALENT: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TALENT: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TALENT: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // TALENT: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x04a41160b06d61c48fa7506cc9dd6761a21850b78589acfdf62b5f8851679315");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Investors applaud rejection of Qatars low Canary Wharf bid 08112014 Independent";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1415416338;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 724005;

        if (fTestNet)
        {
            block.nTime    = 1415416333;
            block.nNonce   = 1346587;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x90031483cc6163ff6d59534e37eab9919b1d63f83b631df279a39e9ac05b8a1d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TALENT: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TALENTMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TALENTMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TALENTMiner(CWallet *pwallet)
{
    printf("TALENTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("TALENT-miner");
    ...
    try { loop {
        ...
        printf("Running TALENTMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TALENTMiner, pwallet));
}
